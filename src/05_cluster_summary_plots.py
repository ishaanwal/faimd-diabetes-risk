...
# Main clustering to interpret (matches your chosen setup)
CLUSTER_COL = "cluster_no_demographics_k4"

# Use the same “no demographics” feature set used for clustering (17 features)
PROFILE_FEATURES = [
...
def load_merged():
    # Load assignments (row-level cluster labels + Diabetes_012)
    assign = pd.read_csv(ASSIGN_PATH)

    if CLUSTER_COL not in assign.columns:
        raise ValueError(
            f"'{CLUSTER_COL}' not found in {ASSIGN_PATH.name}. "
            f"Available cluster columns: {[c for c in assign.columns if c.startswith('cluster_')]}"
        )

    # Load dataset in same order as file 04 (dedup + reset index)
    df = pd.read_csv(DATA_PATH).drop_duplicates().reset_index(drop=True)
    df["row_id"] = np.arange(len(df))

    # Merge cluster labels onto df
    df = df.merge(assign[["row_id", CLUSTER_COL, "Diabetes_012"]], on="row_id", how="inner")
    df = df.rename(columns={CLUSTER_COL: "cluster"})
    return df

def plot_stacked_prevalence(df):
    # Category A: non-diabetic (0) vs pre+diab (>0)
    df["cat_non"] = (df["Diabetes_012"] == 0).astype(int)
    df["cat_pre_or_diab"] = (df["Diabetes_012"] > 0).astype(int)

    # Category B: non+pre (0 or 1) vs diabetic (2)
    ...
    # Sort clusters by higher risk (pre+diab) for readability
    a = a.sort_values("cat_pre_or_diab", ascending=False)
    b = b.loc[a.index]  # keep same order

    # --- Plot 1: Non vs (Pre+Diab)
    ...
    # --- Plot 2: (Non+Pre) vs Diabetic only
    plt.figure(figsize=(9, 5))
    x = np.arange(len(b.index))
    plt.bar(x, b["cat_non_or_pre"], label="Non-diabetic or Prediabetes (Diabetes_012=0 or 1)")
    plt.bar(x, b["cat_diab_only"], bottom=b["cat_non_or_pre"], label="Diabetes only (Diabetes_012=2)")
    ...
    # Save the underlying tables too (useful for report)
...
def plot_cluster_meaning(df):
    # Build mean feature profile per cluster
    feats = [c for c in PROFILE_FEATURES if c in df.columns]
    prof = df.groupby("cluster")[feats].mean()

    # Standardise across clusters (z-score per feature)
    scaler = StandardScaler()
    prof_z = pd.DataFrame(scaler.fit_transform(prof), index=prof.index, columns=prof.columns)

    # Order clusters by Pre+Diab prevalence so it aligns with the prevalence charts
    preplus = (df.assign(preplus=(df["Diabetes_012"] > 0).astype(int))
                 .groupby("cluster")["preplus"].mean()
                 .sort_values(ascending=False))
    prof = prof.loc[preplus.index]
    prof_z = prof_z.loc[preplus.index]

    # --- Plot 3: Heatmap of standardised profiles (this is the best “what cluster represents” view)
    ...
    # --- Plot 4: Bar chart of top differentiating features (readable, helps labeling)
    # Pick top N features with largest spread across clusters (by z-score range)
    ...
    width = 0.09  # for up to ~8 features
...
def main():
    ...
    plot_stacked_prevalence(df)
    plot_cluster_meaning(df)
    ...
    print("\nAlso saved tables to:", OUT_TABLES)
